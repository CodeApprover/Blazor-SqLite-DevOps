...
EXIT_MESSAGES=(
  ...
  [1]="Error locating .config file."
  [2]="JQ not installed."
  [3]="Error with JSON format in .config file."
...
# Extract branches
mapfile -t BRANCHES < <(echo "$JSON_CONFIG" | jq -r '.Branches[]')
main="${BRANCHES[0]}"

# Extract user info
declare -A USER_INFO
for branch in "${BRANCHES[@]}"; do
  read -r USER_NAME USER_EMAIL < <(echo "$JSON_CONFIG" | jq -r ".Users.\"$branch\" | .name, .email")
...
Example:  $0 $main 3 600

1. Mandatory Branch Parameter:
   ${BRANCHES[1]} | ${BRANCHES[2]} | ${BRANCHES[3]}
2. Optional number of pushes, default is 1, max is $MAX_PUSHES.
3. Optional wait seconds, default is 0, max is $MAX_SECS_WAIT seconds.
EOM
)
